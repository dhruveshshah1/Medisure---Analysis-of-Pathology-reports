...
while(alive == True):
    ...
    print("Device: ", client_address)
    try:
        command = connection.recv(1000000)

        search = command.decode("utf-8")

        search = search.replace('\n', " ")
        print(search)
        x=search
        p = []
        p.append(re.compile(r'(?s)HAEMOGLOBIN(.*?)g|o'))
        p.append(re.compile(r'(?s)RBC COUNT(.*?)M'))
        p.append(re.compile(r'(?s)P.C|PCV(.*?)MALE'))
        ...
        chart = ["IRON Deficiency", "B12 Deficiency", "Lukemia", "Anemia", "Polycythemia Vera", "Genetic Anemia",
                 "Infection/Inflammation", "Malaria", "Dengue", "Aids", "Thalassaemia", "Bone Marrow", "Jaundice",
                 "Blood  Tranfusion", "NONE"]
        for i in range(len(target[0])):
            print("{} : {:.2f}%".format(chart[i], target[0][i] * 100))
        # pattern = re.compile(r'Test')
        # pattern2 = re.compile(r'Result')
        #
        # pattern3 = re.compile(r'TEST\s+([a-zA-Z\:\s])+DIFFERENTIAL COUNT')
        # pattern4 = re.compile(r'PATIENT\\'s VALUES\s+([0-9.\s])+Unit')
        #
        # OR WE CAN USE A SIMPLER VERSION
        #
        # pattern3 = re.compile(r'(?s)Test(.*?)Result')
        # pattern4 = re.compile(r'(?s)Result(.*?)Unit|g/di')

        # result = pattern3.finditer(search)
        # names = ''
        # for r in result:
        #     names = r.group()
        #
        # values = ''
        # x = pattern4.finditer(search)
        #
        # for i in x:
        #     values = i.group()
        #
        # names = re.sub(r'^\W*\w+\W*', '', names)
        # values = re.sub(r'^\W*\w+\W*', '', values)
